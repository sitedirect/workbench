...
class PackageCreator {
    ...
    protected function writeSupportDirectory(Package $package, $support, $directory)
    {
        ...
        $path = $directory.DIRECTORY_SEPARATOR.$support;
        ...
        switch ($support) {
            ...
            case 'routes':
                ...
                $this->files->put($path.'/api.php', $content);
                break;

            case 'database/factories':
                $content = $this->formatPackageStub($package, $this->files->get(__DIR__.'/stubs/ModelFactory.stub'));
                $this->files->put($path.'/ModelFactory.php', $content);
                break;

            case 'database/seeds':
                $content = $this->formatPackageStub($package, $this->files->get(__DIR__.'/stubs/TableSeeder.stub'));
                $this->files->put($path.'/'.$package->singular.'.php', $content);
                break;
    ...
    protected function writeProviderStub(Package $package, $directory, $stub)
    {
        ...
        $file = $path.'/'.$package->singular.'ServiceProvider.php';
    ...
    protected function writeSourceClassDirectory(Package $package, $classDirectory, $directory)
    {
        ...
        switch ($classDirectory) {
            ...
            case 'Http/Controllers':
                ...
                $this->files->put($path.'/'.$package->name.'Controller.php', $content);
                break;
    ...
    protected function writeMainClassFile(Package $package, $directory)
    {
        ...
        $this->files->put($classDirectoryPath.'/'.$package->singular.'.php', $content);
    }
